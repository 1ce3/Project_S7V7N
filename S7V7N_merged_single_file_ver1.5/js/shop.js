...
import { loadProductsFromFirebase, loadOutletProductsFromFirebase } from './loader.js';
    ...
    return `
        <div class="bg-gray-800 rounded-lg shadow-lg overflow-hidden flex flex-col relative">
            ...
            <div class="p-4 flex flex-col flex-grow">
                ...
                <div class="mt-auto">
                    <button onclick="addToCart('${product.id}', 1, '${source}')" class="w-full py-2 px-4 rounded-lg text-sm font-semibold transition-colors btn-scale ${isOutOfStock ? 'bg-gray-500 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-500 text-white'}" ${isOutOfStock ? 'disabled' : ''}>
...
export async function showProductDetail(productId, source = 'main') {
    ...
    const staticImg = document.getElementById('product-image');
    const toggleBtn = document.getElementById('toggle-360-btn');

    // Reset Viewer State
    viewer.classList.add('hidden');
    staticImg.classList.remove('hidden');
    toggleBtn.classList.add('hidden');
    toggleBtn.textContent = "360°";

    // Clone viewer to remove old event listeners
    const newViewer = viewer.cloneNode(true);
    viewer.parentNode.replaceChild(newViewer, viewer);
    const currentViewer = document.getElementById('product-360-viewer');

    if (product.spriteSheet) {
        toggleBtn.classList.remove('hidden');
        
        // === 360 Viewer Configuration ===
        // DEFINE YOUR SOURCE SPRITE GRID HERE
        // If your sprite sheet is 6000px wide with 10 frames, frameWidth is 600.
        // Adjusting this value slightly (e.g. 601 or 599) can fix "drifting" issues.
        const sourceFrameWidth = 500; 
        const sourceFrameHeight = 500; 
        const frameCount = 8;       
        
        // Viewer Logic
        currentViewer.style.backgroundImage = `url(${product.spriteSheet})`;
        currentViewer.style.backgroundRepeat = 'no-repeat';

        // Function to calculate sizes based on source pixels + scaling
        const updateViewerSize = () => {
            if (currentViewer.classList.contains('hidden')) return;
            
            const containerW = currentViewer.clientWidth;
            const containerH = currentViewer.clientHeight;
            
            if (containerW === 0) return;

            // Calculate Scale Ratio to fit width
            const scale = containerW / sourceFrameWidth;
            
            // Calculate final background size
            // Width = (SourceWidth * Count) * Scale
            // Height = SourceHeight * Scale (or just '100%' to force fit)
            const finalBgWidth = (sourceFrameWidth * frameCount) * scale;
            const finalBgHeight = sourceFrameHeight * scale; // Maintains aspect ratio
            
            // Apply
            currentViewer.style.backgroundSize = `${finalBgWidth}px ${finalBgHeight}px`;
            
            // Store scale for position updates
            currentViewer.dataset.scale = scale;
        };

        // Toggle Action
        toggleBtn.onclick = () => {
            const isHidden = currentViewer.classList.contains('hidden');
            if (isHidden) {
                currentViewer.classList.remove('hidden');
                staticImg.classList.add('hidden');
                toggleBtn.textContent = "Show Image";
                requestAnimationFrame(() => updateViewerSize());
            } else {
                currentViewer.classList.add('hidden');
                staticImg.classList.remove('hidden');
                toggleBtn.textContent = "360°";
            }
        };
        ...
        const updateFrame = (x) => {
            ...
            const scale = parseFloat(currentViewer.dataset.scale || 1);

            // Calculate percentage
            const percent = Math.max(0, Math.min(1, (x - rect.left) / rect.width));
            const frameIndex = Math.floor(percent * frameCount);
            const clampedIndex = Math.min(frameCount - 1, Math.max(0, frameIndex));
            
            // Calculate Offset in Source Pixels, then Scale
            const xOffset = -(clampedIndex * sourceFrameWidth * scale);
            
            currentViewer.style.backgroundPosition = `${xOffset}px center`;
        };

        let isDragging = false;

        // Mouse Events
        currentViewer.onmousedown = (e) => { isDragging = true; updateFrame(e.clientX); e.preventDefault(); };
        currentViewer.onmousemove = (e) => { if (isDragging) updateFrame(e.clientX); };
        currentViewer.onmouseup = () => { isDragging = false; };
        currentViewer.onmouseleave = () => { isDragging = false; };

        // Touch Events
        currentViewer.ontouchstart = (e) => { isDragging = true; updateFrame(e.touches[0].clientX); };
        currentViewer.ontouchmove = (e) => { 
            if (isDragging) {
                updateFrame(e.touches[0].clientX); 
                e.preventDefault(); // Prevent scrolling while swiping
            }
        };
        currentViewer.ontouchend = () => { isDragging = false; };

        // Initialize
        currentViewer.style.backgroundPosition = `0px center`;
    }
...
export function filterAndRender() {
    ...
    const searchTerm = document.getElementById('shop-search-bar')?.value.toLowerCase() || '';
...
export function filterAndRenderOutlet() {
    ...
    const searchTerm = document.getElementById('outlet-search-bar')?.value.toLowerCase() || '';
...
// === PERFORMANCE LAB LOGIC ===
let currentLabProduct = null;
let currentResolution = '1080p';

// 2. Initialize the Lab (Updated to use Firebase)
export async function initPerformanceLab(product) {
    const container = document.getElementById('performance-lab-container');
    
    // Check if PC Case
    if (product.category !== 'PC Case' || !product.specLevels) {
    ...
    let labGames = [];
    try {
        const snap = await db.collection('performance_games').get();
        labGames = snap.docs.map(d => d.data());
    } catch (e) {
        console.error("Could not load games", e);
    }

    // Fallback if database is empty (so it doesn't look broken)
    if (labGames.length === 0) {
        labGames = [
            { id: 'cs2', name: 'Counter-Strike 2', difficulty: 3, image: 'https://placehold.co/300x400?text=CS2' }
        ];
    }

    container.classList.remove('hidden');
    currentLabProduct = product; // Set global state

    // Make labGames available globally for the calculator
    window.currentLabGames = labGames; 

    // Render Cards
    const selector = document.getElementById('lab-game-selector');
    selector.innerHTML = labGames.map((game) => `
        <div onclick="calculateFPS('${game.id}')" 
             class="relative flex-shrink-0 w-32 h-40 rounded-lg overflow-hidden cursor-pointer border-2 border-transparent hover:border-indigo-500 transition-all group snap-start shadow-lg">
            <img src="${game.image || game.icon || 'https://placehold.co/300x400?text=' + game.name}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" alt="${game.name}">
            <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent flex items-end p-3">
                <span class="font-bold text-white text-sm leading-tight group-hover:text-indigo-400 transition-colors drop-shadow-md">${game.name}</span>
            </div>
            <div class="absolute top-1 right-1 bg-black/60 backdrop-blur text-[10px] font-bold text-white px-1.5 py-0.5 rounded border border-white/20">
                ${game.difficulty}
            </div>
        </div>
    `).join('');
    ...
    // Select first game
    if (labGames.length > 0) calculateFPS(labGames[0].id);
}

// 3. Make these global so HTML can call them
window.setResolution = (res) => {
    ...
    ['1080p', '1440p', '4k'].forEach(r => {
        ...
        if(r === res) {
            btn.className = "px-3 py-1 text-xs font-bold rounded bg-indigo-600 text-white transition-all";
        } else {
            btn.className = "px-3 py-1 text-xs font-bold rounded text-gray-400 hover:text-white transition-all";
        }
    });

    // Recalculate if a game is selected
    const activeGameName = document.getElementById('lab-game-title').textContent;
    const activeGame = window.currentLabGames.find(g => g.name === activeGameName);
    if(activeGame) calculateFPS(activeGame.id);
};

window.calculateFPS = (gameId) => {
    if (!currentLabProduct || !currentLabProduct.specLevels) return;

    const game = window.currentLabGames.find(g => g.id === gameId);
    
    // Get PC Specs (Default to 1 if missing)
    const cpuScore = currentLabProduct.specLevels.cpu || 1;
    const gpuScore = currentLabProduct.specLevels.gpu || 1;
    const ramScore = currentLabProduct.specLevels.ram || 1;
    // Calculate Hardware Power (GPU matters more for gaming)
    const pcPower = (cpuScore * 0.3) + (gpuScore * 0.5) + (ramScore * 0.2); 
    
    // Resolution Penalty
    let resFactor = 1;
    if (currentResolution === '1440p') resFactor = 0.75;
    if (currentResolution === '4k') resFactor = 0.5;

    // The Algo: Power vs Difficulty
    // A Power 5 PC matches a Difficulty 5 Game at ~60 FPS
    let baseFPS = 120;
    let ratio = pcPower / game.difficulty;
    
    // Curve the result (Exponential returns for over-qualified hardware)
    let estimatedFPS = Math.round(baseFPS * ratio * resFactor);
    
    // Cap unrealistic numbers
    if (estimatedFPS > 999) estimatedFPS = 999;
    
    // Update UI
    document.getElementById('lab-game-title').textContent = game.name;
    
    const fpsEl = document.getElementById('lab-fps-number');
    const barEl = document.getElementById('lab-fps-bar');
    
    // Animate Number
    let current = 0;
    const step = Math.ceil(estimatedFPS / 20);
    const timer = setInterval(() => {
        current += step;
        if (current >= estimatedFPS) {
            current = estimatedFPS;
            clearInterval(timer);
        }
        fpsEl.textContent = current;
        
        // Color Coding
        if (current < 30) fpsEl.className = "text-5xl font-black text-red-500";
        else if (current < 60) fpsEl.className = "text-5xl font-black text-yellow-500";
        else fpsEl.className = "text-5xl font-black text-green-500";
    }, 20);

    // Update Bar Width (Max 144 for visual scaling)
    const percentage = Math.min((estimatedFPS / 240) * 100, 100);
