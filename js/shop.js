...
/**
 * Filters products by category and price, then renders them to the grid.
 * This is the central function for the category page.
 */
function filterAndRender() {
    const grid = document.getElementById('category-product-grid');
    if (!grid) return; // Exit if not on the category page
    ...
    document.querySelector('#category-breadcrumb .text-white').textContent = category === 'All' ? 'All Products' : category;
    document.getElementById('price-value').textContent = formatCurrency(maxPrice);
...
/**
 * Event handler for the price slider. It calls the main filter function.
 */
...
/**
 * Initializes the category page by setting the current category and calling the filter function.
 */
...
export function initProductPage() {
    ...
    if (!page) return; // Exit if not on the product page
    ...
    if (!product) {
        page.innerHTML = `<h1 class="text-2xl text-red-500">Product not found.</h1><a href="index.html" class="text-indigo-400">Go back home</a>`;
        return;
    }

    document.title = `S7V7N - ${product.name}`;
    document.getElementById('product-image').src = product.image || 'https://placehold.co/600x600?text=No+Image';
